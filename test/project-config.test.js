...
for (const [name, config, message] of [
  ...
  ['standardsProfile desconhecido', { schemaVersion: 1, template: 'traditional-web', standardsProfile: 'traditional-web-unknown', tools: {} }, 'standardsProfile da configuração do projeto não é suportado para o template'],
...
test('createProjectConfig não muta input nem argsPrefix', () => {
  ...
  assert.equal(config.standardsProfile, 'traditional-web-v4')
