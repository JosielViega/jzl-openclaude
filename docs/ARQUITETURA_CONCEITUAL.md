...
O Project Config separa a família `template` do contrato versionado `standardsProfile`. Novos projetos `traditional-web` são explicitamente pinned no profile inicial `traditional-web-v3`; configurações legacy sem o campo permanecem válidas e resolvem permanentemente para o profile legacy `traditional-web-v1`, nunca para um conceito de “latest”. A evolução até v3 demonstra que profiles pinned não são alterados pela evolução do JZL.
