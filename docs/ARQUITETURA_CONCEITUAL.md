...
O perfil `traditional-web-v3` inclui integralmente o contrato v2 e acrescenta o Public Exposure Contract v1. Um document root padronizado deve conter somente artefatos deliberadamente expostos, e essa decisão deve ser verificável por path sem inspecionar segredos. Dentro de `public/`, paths de controle (`.jzl`, `.git`, `.openclaude`), dependências (`vendor`, `node_modules`) e ambiente (`.env`, `.env.*`) são proibidos em qualquer profundidade; manifests Composer/npm selecionados são proibidos somente na raiz pública. Dotfiles genéricos e `.well-known` não são banidos. Links simbólicos e junctions não são seguidos, e o Structure Standard continua responsável pela validade do próprio `public/`.

O perfil `traditional-web-v4` inclui integralmente o v3 e acrescenta o Technology Boundary v1. A stack fonte first-party baseline é PHP, SQL, JavaScript, HTML e CSS. Arquivos regulares com extensões `.ts`, `.tsx`, `.jsx`, `.vue`, `.svelte`, `.py`, `.go`, `.java`, `.cs` ou `.rb` são detectados independentemente de casing, usando a discovery existente sem ler conteúdo ou seguir links. Arquivos neutros não são banidos genericamente. Os contratos v1, v2 e v3 permanecem congelados.

Após o traditional-web-v4, o conjunto principal de standards é considerado suficiente para o uso pessoal atual; novas regras devem surgir de necessidades observadas em projetos reais, não de antecipação arquitetural.
...
O Project Config separa a família `template` do contrato versionado `standardsProfile`. Novos projetos `traditional-web` são explicitamente pinned no profile inicial `traditional-web-v4`; configurações legacy sem o campo permanecem válidas e resolvem permanentemente para o profile legacy `traditional-web-v1`, nunca para um conceito de “latest”. A evolução até v4 demonstra que profiles pinned não são alterados pela evolução do JZL.
